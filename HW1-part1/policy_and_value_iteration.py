...
def policy_evaluation(env, V, policy, gamma, eps):
        ...
        for a in range(num_actions):
            for prob, next_state, reward, done in env.P[s][a]:
                if done:
                    ...
                    q_s[a] += (prob * (reward + gamma * V[next_state]))
        policy_new = np.argmax(q_s)
        if policy_new != policy[s]:
            stable = False
        policy[s] = policy_new
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    for _ in range(max_iterations):  
        policy_evaluation(env, V, policy, gamma, eps)
        stable = policy_improvement(env, V, policy, gamma)
        if stable:
            # print(_)
            break
